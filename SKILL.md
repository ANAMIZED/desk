...
Agent card: https://anamized.grok.me/.well-known/agent-card.json (also /.well-known/agent.json). JSON-RPC: POST https://anamized.grok.me/a2a. Methods: `message/send`, `tasks/get`, `tasks/cancel`. Free skills: catalog, floor, rack, join. Paid skills return task state `auth-required` plus x402 terms until a claimed Bearer is attached.
